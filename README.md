...
| `items`                                         | The items for the component to render<br> - Each item should be in the following format:<br>`{label: 'Orange', value: 'orange', key: 'orange', color: 'orange', displayValue: true, scrollLabel: 'Orange color'}`<br>- `label` and `value` are required<br>- `key`, `color`, `displayValue`, and `scrollLabel` are optional<br>- `key` will be set to equal `label` if not included<br>- `value` can be any data type<br>- If `displayValue` is truthy, the TextInput will display the `value` instead of the `label`<br>- If scrollLabel is truthy, its value will be displayed in the native picker list. The selected value will still use label. If scrollLabel is falsy, label will be used.                               | **required**<br>array    |
